...
public:
	...
	void Clip(PointF* input, int nin, PointF *clipped, int *nc)
	{
		*nc=0;
		m_stageOut.SetDestination(clipped, nc);

		// Clip each input vertex.
		for (int i=0; i<nin; ++i) 
			m_stageRight.HandleVertex(input[i]);
...
private:
	...
	public:
		...
		void SetDestination(PointF * pDest, int *nc) { m_pDest = pDest; m_nc=nc; }
		void HandleVertex(const PointF& pnt)	{ m_pDest[*m_nc]=pnt; ++(*m_nc); }	// Append the vertex to the output container.
